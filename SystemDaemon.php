...
abstract class SystemDaemon
{
    ...
    public function start()
    {
        ...
        $pid = pcntl_fork();
        if ($pid < 0) {
            fprintf(STDERR, "fork failure: %s", posix_strerror(posix_get_last_error()));
            exit;
        } elseif ($pid) {
            /**
            ...
            if ($this->sid < 0) {
                fprintf(STDERR, "setsid() failure: %s", posix_strerror(posix_get_last_error()));
                exit;
            }
            ...
            if (!$this->log->check()) {
                fprintf(
                    STDERR,
                    "FATAL: could not open logfile '%s': %s",
                    $this->logfile,
    ...
    /**
     * We also support non-catched exceptions and consider them as fatal errors.
     *
